...
type EmulatorGateway struct {
	...
	sdkEmulator     *adapters.SDKAdapter
	ctx             context.Context
...
func NewEmulatorGatewayWithOpts(key *EmulatorKey, opts ...func(*EmulatorGateway)) *EmulatorGateway {
	...
	gateway.sdkEmulator = adapters.NewSDKAdapter(gateway.logger, gateway.emulator)
	gateway.sdkEmulator.EnableAutoMine()
...
func (g *EmulatorGateway) GetAccount(address flow.Address) (*flow.Account, error) {
	account, err := g.sdkEmulator.GetAccount(g.ctx, address)
	if err != nil {
...
func (g *EmulatorGateway) SendSignedTransaction(tx *flow.Transaction) (*flow.Transaction, error) {
	err := g.sdkEmulator.SendTransaction(context.Background(), *tx)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
	return tx, nil
...
func (g *EmulatorGateway) GetTransactionResult(ID flow.Identifier, _ bool) (*flow.TransactionResult, error) {
	result, err := g.sdkEmulator.GetTransactionResult(g.ctx, ID)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
	return result, nil
...
func (g *EmulatorGateway) GetTransaction(id flow.Identifier) (*flow.Transaction, error) {
	transaction, err := g.sdkEmulator.GetTransaction(g.ctx, id)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
...
func (g *EmulatorGateway) executeScriptQuery(
	...
	if query.id != flow.EmptyID {
		result, err = g.sdkEmulator.ExecuteScriptAtBlockID(g.ctx, query.id, script, args)
	} else if query.height > 0 {
		result, err = g.sdkEmulator.ExecuteScriptAtBlockHeight(g.ctx, query.height, script, args)
	} else {
		result, err = g.sdkEmulator.ExecuteScriptAtLatestBlock(g.ctx, script, args)
	}
...
func (g *EmulatorGateway) GetLatestBlock() (*flow.Block, error) {
	block, _, err := g.sdkEmulator.GetLatestBlock(g.ctx, true)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
...
func (g *EmulatorGateway) getBlockEvent(height uint64, eventType string) flow.BlockEvents {
	block, _, _ := g.sdkEmulator.GetBlockByHeight(g.ctx, height)
	events, _ := g.sdkEmulator.GetEventsForBlockIDs(g.ctx, eventType, []flow.Identifier{block.ID})
...
func (g *EmulatorGateway) GetCollection(id flow.Identifier) (*flow.Collection, error) {
	collection, err := g.sdkEmulator.GetCollectionByID(g.ctx, id)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
...
func (g *EmulatorGateway) GetBlockByID(id flow.Identifier) (*flow.Block, error) {
	block, _, err := g.sdkEmulator.GetBlockByID(g.ctx, id)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
	return block, nil
...
func (g *EmulatorGateway) GetBlockByHeight(height uint64) (*flow.Block, error) {
	block, _, err := g.sdkEmulator.GetBlockByHeight(g.ctx, height)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
	return block, nil
...
func (g *EmulatorGateway) GetLatestProtocolStateSnapshot() ([]byte, error) {
	snapshot, err := g.sdkEmulator.GetLatestProtocolStateSnapshot(g.ctx)
	if err != nil {
		return nil, UnwrapStatusError(err)
	}
	return snapshot, nil
