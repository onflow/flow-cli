...
func (s *Services) GetAccount(address flow.Address) (*flow.Account, error) {
	s.logger.StartProgress(fmt.Sprintf("Loading %s...", address))

	account, err := s.gateway.GetAccount(address)
...
func (s *Services) CreateAccount(
	...
	for _, contract := range contractArgs {
		...
		contractSource, err := s.state.ReadFile(contractFlagContent[1])
		if err != nil {
			return nil, err
		}
	...
	tx, err = s.prepareTransaction(tx, signer)
	if err != nil {
		return nil, err
	}

	s.logger.Info(fmt.Sprintf("Transaction ID: %s", tx.FlowTransaction().ID()))
	s.logger.StartProgress("Creating account...")
	defer s.logger.StopProgress()

	sentTx, err := s.gateway.SendSignedTransaction(tx)
	if err != nil {
		return nil, err
	}

	s.logger.StartProgress("Waiting for transaction to be sealed...")

	result, err := s.gateway.GetTransactionResult(sentTx, true)
	if err != nil {
		return nil, err
	}
	...
	return s.gateway.GetAccount(*newAccountAddress[0]) // we know it's the only and first event
}

// AddContract deploys a contract code to the account provided with possible update flag.
func (s *Services) AddContract(
	...
	tx, err = s.prepareTransaction(tx, account)
	if err != nil {
		return nil, err
	}
	...
	trx, err := s.gateway.GetTransactionResult(sentTx, true)
	if err != nil {
		return nil, err
	}

	if trx.Error != nil {
		s.logger.Error("Failed to deploy contract")
		return nil, trx.Error
	}

	update, err := s.gateway.GetAccount(account.Address())
...
func (s *Services) RemoveContract(
	account *flowkit.Account,
	contractName string,
) (*flow.Account, error) {
	...
	s.logger.Info(fmt.Sprintf("Transaction ID: %s", tx.FlowTransaction().ID().String()))
	s.logger.StartProgress(
	...
	txr, err := s.gateway.GetTransactionResult(sentTx, true)
	if err != nil {
		return nil, err
	}
	if txr != nil && txr.Error != nil {
		s.logger.Error("Removing contract failed")
	...
	return s.gateway.GetAccount(account.Address())
}

// prepareTransaction prepares transaction for sending with data from network
func (s *Services) prepareTransaction(
	...
	proposer, err := s.gateway.GetAccount(account.Address())
	if err != nil {
		return nil, err
	}
